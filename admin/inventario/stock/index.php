...
<main class="admin-main" id="contenido-principal">
    <?php if ($fractionable && $currentStock > 0 && $activeLotCount === 0): ?>
        <div class="admin-alert admin-alert--warning" role="alert"><strong>Stock antiguo sin lotes</strong>
            <p>Este alimento seco tiene stock histórico. No se recalculó ni eliminó; regularízalo con lotes antes de vender
                por presentación.</p>
        </div>
    <?php endif; ?>
    ...
    <section class="admin-stock-module" aria-labelledby="stock-module-title">
        ...
        <div class="admin-stock-overview">
            ...
            <div class="admin-stock-overview__item">
                ...
                <div class="admin-stock-overview__status">
                    <span class="admin-status-badge <?= escape(
                        $statusClass
                    ) ?>">
                        <?= escape($stockStatus) ?>
                    </span>
        ...
        <section class="admin-stock-section" aria-labelledby="movement-form-title">
            <header class="admin-stock-section__header">
                <div>
                    ...
                    <p>
                        <?= $fractionable
                            ? 'Entrada suma peso mediante lotes y salida descuenta presentaciones físicas.'
                            : 'Entrada suma unidades, salida descuenta y ajuste establece el stock real final.' ?>
                    </p>
                </div>
            </header>

            <form id="form-movimiento-stock" class="admin-product-form" method="post" action="<?= escape(
                appUrl(
                ...
                <?php if ($fractionable && $activeLotCount === 0 && $currentStock > 0): ?>
                    <label class="admin-alert admin-alert--warning"><input id="confirmar_regularizacion" type="checkbox"
                            name="confirmar_regularizacion" value="1" required> Confirmo que esta entrada regularizará el
                        stock histórico y que el nuevo stock vendible quedará determinado por el peso disponible de los
                        lotes.</label>
                <?php endif; ?>

                <div class="admin-stock-form-grid">

                    <div class="admin-field<?= isset(
                        ...
                        <select id="tipo_movimiento" name="tipo_movimiento" required>
                            ...
                            <?php foreach (
                                ...
                                <option value="<?= escape($type) ?>" <?= $values['tipo_movimiento'] === $type ? ' selected' : '' ?>>
                                    <?= escape($label) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div <?= $fractionable ? 'hidden' : '' ?> class="admin-field<?= isset(
                               $errors['cantidad']
                           )
                               ? ' admin-field--invalid'
                               : '' ?>">
                        ...
                        <input id="cantidad" name="cantidad" type="number" inputmode="numeric" min="0" step="1"
                            placeholder="<?= $fractionable ? 'Ej.: 1000' : 'Ej.: 10' ?>" required
                            value="<?= escape((string) $values['cantidad']) ?>" <?php if ($fractionable): ?> disabled
                            <?php endif; ?>>
                    ...
                    <div class="admin-field">
                        <span
                            class="admin-field__help"><?= $fractionable ? 'Ejemplo: 1 kg = 1000 g, 250 g = 250.' : 'Ingresa una cantidad entera de unidades.' ?></span>
                    </div>

                    <?php if ($fractionable): ?>
                        <div class="admin-field admin-field--full" id="salida-presentacion-fields" hidden>
                            <label for="id_presentacion">Presentación de venta</label>
                            <select id="id_presentacion" name="id_presentacion">
                                <option value="">Seleccionar presentación</option>
                                <?php foreach ($presentations as $presentation): ?>
                                    <option value="<?= (int) $presentation['id_presentacion'] ?>"
                                        data-grams="<?= (int) $presentation['cantidad_gramos'] ?>">
                                        <?= escape(
                                            (string) $presentation['nombre']
                                            . ' · '
                                            . formatearCantidadStock(
                                                (int) $presentation['cantidad_gramos'],
                                                true
                                            )
                                        ) ?>
                                    </option>
                        ...
                        <div class="admin-field admin-field--full admin-lots-panel admin-lots-panel--stock"
                            id="entrada-lotes-fields" hidden>
                            <div class="admin-lots-panel__header"><span class="admin-lots-panel__icon"><i
                                        class="bi bi-box-seam" aria-hidden="true"></i></span>
                                <div><span class="admin-lots-panel__badge">Solo alimento seco</span>
                                    <h4>Lotes de stock</h4>
                                    <p>Registra el origen, vencimiento y peso real de esta entrada.</p>
                                </div>
                            </div>
                            <div class="admin-field admin-lots-provider"><label for="id_proveedor"><i class="bi bi-truck"
                                        aria-hidden="true"></i> Proveedor</label><select id="id_proveedor"
                                    name="id_proveedor">
                                    <option value="">Asignar después</option><?php foreach ($suppliers as $supplier): ?>
                                ...
                                </select><?php if ($suppliers === []): ?><span class="admin-field__help"><i
                                            class="bi bi-info-circle" aria-hidden="true"></i> Aún no hay proveedores
                                        registrados.</span><?php else: ?><span class="admin-field__help">Opcional. Puedes
                                        asignarlo ahora o dejarlo para después.</span><?php endif; ?></div>
                            <div id="lotes-container" class="admin-lots-list"></div>
                            <button class="admin-button admin-lots-add" type="button" id="agregar-lote"><i
                                    class="bi bi-plus-circle" aria-hidden="true"></i> Agregar lote</button>
                        </div>
                        <template id="lote-template">
                            <fieldset class="admin-lot-card" data-lote>
                                <legend><span class="admin-lot-card__number"><i class="bi bi-box"
                                            aria-hidden="true"></i></span><span><strong>Nuevo
                                            lote</strong><small>Identificación, fechas y peso recibido</small></span></legend>
                                <div class="admin-lot-fields"><label class="admin-field admin-lot-code">Código de lote<input
                                            data-name="codigo_lote" maxlength="80" required
                                            placeholder="Ej.: LT-2026-001"></label>
                                    <label class="admin-field">Fecha elaboración<input data-name="fecha_elaboracion"
                                            type="date"></label>
                                    <label class="admin-field">Fecha vencimiento<input data-name="fecha_vencimiento"
                                            type="date" required></label>
                                    <label class="admin-field admin-lot-quantity">Cantidad total (g)<input
                                            data-name="cantidad_total_g" type="number" min="0.001" step="0.001" required
                                            placeholder="0"></label>
                                </div>
                                <div class="admin-lot-card__footer">
                                    <p><i class="bi bi-info-circle" aria-hidden="true"></i>El peso estará disponible para todos los formatos de venta compatibles.</p>

                                    <button type="button" class="admin-button admin-button--small" data-quitar-lote>
                    ...
                    <div class="admin-field admin-stock-form-grid__full<?= isset(
                        $errors['observacion']
                    )
                        ? ' admin-field--invalid'
                        : '' ?>">
                        ...
                        <textarea id="observacion" name="observacion" maxlength="150" rows="3"
                            <?= $values['motivo'] === 'otro' ? 'required' : '' ?>><?= escape(
                ...
                <div class="admin-stock-actions">
                    <a class="admin-stock-actions__back" href="<?= escape(
                        appUrl('admin/inventario/index.php')
                    ) ?>">
                        ← Volver al inventario
                    </a>
            ...
            <template id="stock-confirm-template">
                <div class="admin-stock-confirm-summary" data-stock-confirm-summary>
                    <div class="admin-stock-confirm-summary__row"><span
                            class="admin-stock-confirm-summary__label">Producto</span><strong
                            class="admin-stock-confirm-summary__value" data-stock-summary-product></strong></div>
                    <div class="admin-stock-confirm-summary__row"><span class="admin-stock-confirm-summary__label">Tipo
                            de movimiento</span><strong class="admin-stock-confirm-summary__value"
                            data-stock-summary-type></strong></div>
                    <div class="admin-stock-confirm-summary__row"><span
                            class="admin-stock-confirm-summary__label">Cantidad</span><strong
                            class="admin-stock-confirm-summary__value" data-stock-summary-quantity></strong></div>
                    <div class="admin-stock-confirm-summary__row"><span
                            class="admin-stock-confirm-summary__label">Motivo</span><strong
                            class="admin-stock-confirm-summary__value" data-stock-summary-reason></strong></div>
                    <div class="admin-stock-confirm-summary__row" data-stock-summary-observation-row><span
                            class="admin-stock-confirm-summary__label">Observación</span><strong
                            class="admin-stock-confirm-summary__value" data-stock-summary-observation></strong></div>
                </div>
            </template>
            <script>
                (() => {
                    ...
                    <?php if ($fractionable): ?>
                        ...
                        const renumerar = () => [...container.children].forEach((lote, i) => { lote.querySelectorAll('[data-name]').forEach(el => el.name = `lotes[${i}][${el.dataset.name}]`); });
                        const agregar = () => { const lote = template.content.firstElementChild.cloneNode(true); container.append(lote); lote.querySelector('[data-quitar-lote]').onclick = () => { lote.remove(); renumerar(); }; renumerar(); };
        ...
        <section class="admin-stock-history" aria-labelledby="stock-history-title">
            <header class="admin-stock-section__header">
                <div>
