...
<main class="admin-main admin-blog blog-edit" id="contenido-principal">
    ...
    <div class="admin-form-layout admin-product-edit-shell">
        <form class="admin-form-layout__form admin-product-edit-form blog-edit__form" method="post" enctype="multipart/form-data" action="<?= escape(appUrl('admin/blog/actualizar.php')) ?>">
            ...
            <section class="admin-panel admin-form-panel blog-edit__fields" aria-labelledby="blog-edit-data-title">
                ...
                <div class="admin-form-grid admin-blog-data-grid">
                    ...
                    <div class="admin-field admin-blog-field--category<?= isset($errors['id_categoria_blog']) ? ' admin-field--invalid' : '' ?>">
                        ...
                        <select id="id_categoria_blog" name="id_categoria_blog" required <?= isset($errors['id_categoria_blog']) ? 'aria-invalid="true" aria-describedby="categoria-error"' : '' ?>>
                            ...
                            <?php foreach ($categories as $category): ?>
                                ...
                                <option value="<?= escape((string) $category['id_categoria_blog']) ?>" <?= (int) $values['id_categoria_blog'] === (int) $category['id_categoria_blog'] ? 'selected' : '' ?>><?= escape((string) $category['nombre']) ?><?= $categoryIsActive ? '' : ' (inactiva)' ?></option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div class="admin-field admin-blog-field--slug<?= isset($errors['slug']) ? ' admin-field--invalid' : '' ?>">
                        ...
                        <input id="slug" name="slug" type="text" maxlength="210" autocomplete="off" value="<?= escape((string) $values['slug']) ?>" <?= isset($errors['slug']) ? 'aria-invalid="true" aria-describedby="slug-help slug-error"' : 'aria-describedby="slug-help"' ?>>
                    ...
                    <div class="admin-alert admin-blog-draft-notice" role="status">
                        <span class="admin-status-badge <?= escape(claseEstadoBlog((string) $article['estado'])) ?>"><?= escape(etiquetaEstadoBlog((string) $article['estado'])) ?></span>
                ...
                <div class="admin-form-grid">
                    ...
                    <?php if ($currentCoverUrl !== null): ?>
                        <div class="admin-field admin-field--full">
                            <p>Portada actual</p>
                            <img src="<?= escape($currentCoverUrl) ?>" alt="Portada actual de <?= escape((string) $article['titulo']) ?>">
                        </div>
                    <?php endif; ?>
                    <div class="admin-field admin-field--full<?= isset($errors['imagen_portada']) ? ' admin-field--invalid' : '' ?>">
                        <label for="imagen_portada">Reemplazar imagen de portada (opcional)</label>
                        ...
                        <p class="admin-field__help" id="imagen-portada-help">JPG, PNG o WEBP. Máximo 2 MB. Si no seleccionas otra, se conservará la actual.</p>
