...
$clientLoggedIn = isset($_SESSION['id_cliente'])
  && is_numeric($_SESSION['id_cliente']);

$clientName = $clientLoggedIn
  && is_string($_SESSION['cliente_nombre'] ?? null)
  ? $_SESSION['cliente_nombre']
  : '';

$cartUnits = contarUnidadesCarritoSesion();

$cartLabel = $cartUnits === 1
  ? 'Carrito, 1 producto'
  : 'Carrito, ' . $cartUnits . ' productos';
...
<header class="site-header" id="cabecera-publica">
  <div class="container header-inner">
    <a class="brand" href="<?= e(appUrl('index.php')) ?>" aria-label="Ir al inicio de Coratto Pet">
      <img src="<?= e(appUrl('public/assets/img/logo-coratto-navbar.png')) ?>" alt="Coratto Pet" width="600"
        height="200">
    </a>
    <nav class="main-nav" id="main-nav" aria-label="Navegación principal">
      ...
      <div class="nav-dropdown" data-nav-dropdown>
        <button class="main-nav__products<?= $currentPage === 'catalogo' ? ' active' : '' ?>" type="button"
          aria-expanded="false" aria-controls="productos-menu">
          Catálogo
          <svg class="main-nav__products-icon" aria-hidden="true" viewBox="0 0 24 24">
              <path d="m7 9 5 5 5-5" />
          </svg>
        </button>
        ...
        <?php foreach ($publicCategories as $category): ?>
          <a
              class="nav-dropdown__category"
              href="<?= e(appUrl('public/catalogo.php?categoria=' . rawurlencode((string) $category['id']))) ?>"
          >
              <?= e((string) $category['nombre']) ?>
          </a>
        <?php endforeach; ?>
          <a class="nav-dropdown__all" href="<?= e(appUrl('public/catalogo.php')) ?>">Ver todos los productos</a>
        </div>
      </div>
      ...
      <?php if ($clientLoggedIn): ?>
        <div class="mobile-client-actions">
          <a href="<?= e(appUrl('public/clientes/cuenta.php')) ?>">Mi cuenta</a>
    ...
    <div class="header-actions">
      <button class="header-action header-action--search" type="button" aria-label="Buscar productos"
        aria-expanded="false" aria-controls="public-search">
        <svg aria-hidden="true" viewBox="0 0 24 24">
          <circle cx="11" cy="11" r="6.5" />
          <path d="m16 16 4 4" />
        </svg>
      </button>
      <a class="header-action header-action--user"
        href="<?= e(appUrl($clientLoggedIn ? 'public/clientes/cuenta.php' : 'public/clientes/login.php')) ?>"
        aria-label="<?= $clientLoggedIn ? 'Mi cuenta, ' . e($clientName) : 'Crear cuenta o iniciar sesión' ?>">
        <svg aria-hidden="true" viewBox="0 0 24 24">
      ...
      <?php if ($clientLoggedIn): ?>
        <form class="header-logout-form" method="post" action="<?= e(appUrl('public/clientes/logout.php')) ?>">
          <input type="hidden" name="csrf_token" value="<?= e(csrfToken()) ?>">
          <button type="submit">Cerrar sesión</button>
        </form>
      <?php endif; ?>
      <a class="header-action header-cart<?= $currentPage === 'carrito'
        ? ' active'
        : '' ?>" href="<?= e(appUrl('public/carrito.php')) ?>" aria-label="<?= e($cartLabel) ?>">
        <svg aria-hidden="true" viewBox="0 0 24 24">
          <path d="M7 8h10l1 12H6L7 8Z" />
          <path d="M9 9V6a3 3 0 0 1 6 0v3" />
        </svg>

        <span class="header-cart__badge">
          <?= e($cartUnits) ?>
        </span>
      </a>
  ...
  <div class="public-search" id="public-search" hidden>
    <form action="<?= e(appUrl('public/catalogo.php')) ?>" method="get" role="search">
      ...
      <input id="public-search-input" name="buscar" type="search" maxlength="100"
